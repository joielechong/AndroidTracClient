...
public abstract class JSONRPCThreadedClient {
	...
	public static JSONRPCClient create(String uri, Versions version) {
		final JSONRPCClient client = new JSONRPCHttpClient(uri, false);
	...
	public void call(final String method, final OnObjectResultListener listener, final Object... params) {
		...
		final Thread thread = new Thread() {
			...
			public void run() {
				...
				try {
					mo = new MessageObject(Description.NORMAL_RESPONSE, doRequest(method, params).get(JSON_RESULT));
	 ...
	 *             if an error is encountered during JSON-RPC method call
	 */
	public void callString(final String method, final OnStringResultListener listener, final Object... params)
			throws JSONRPCException {
		final Handler handler = new Handler() {
			@Override
			public void handleMessage(Message msg) {
		...
		final Thread thread = new Thread() {
			...
			public void run() {
				MessageObject mo = null;
				try {
					mo = new MessageObject(Description.NORMAL_RESPONSE, doRequest(method, params).getString(JSON_RESULT));
				} catch (final JSONException e) {
					mo = new MessageObject(Description.ERROR, e);
				} catch (final JSONRPCException e) {
					mo = new MessageObject(Description.ERROR, e);
	 ...
	 *             if an error is encountered during JSON-RPC method call
	 */
	public void callString(final String method, final OnStringResultListener listener, final JSONObject params)
			throws JSONRPCException {
		final Handler handler = new Handler() {
			@Override
			public void handleMessage(Message msg) {
		...
		final Thread thread = new Thread() {
			...
			public void run() {
				...
				try {
					...
					mo = new MessageObject(Description.NORMAL_RESPONSE, new Integer(response.getInt(JSON_RESULT)));
				} catch (final JSONException e) {
					try {
						mo = new MessageObject(Description.NORMAL_RESPONSE, new Integer(response.getString(JSON_RESULT)));
		...
		final Handler handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				if (msg.obj == null) {
					...
					final MessageObject mo = (MessageObject) msg.obj;
			...
			public void run() {
				...
				try {
					...
					if (response == null) {
						...
						mo = new MessageObject(Description.NORMAL_RESPONSE, new Long(response.getLong(JSON_RESULT)));
					}
				} catch (final JSONException e) {
					try {
						mo = new MessageObject(Description.NORMAL_RESPONSE, Long.parseLong(response.getString(JSON_RESULT)));
					} catch (final NumberFormatException e1) {
						mo = new MessageObject(Description.ERROR, e1);
		...
		final Thread thread = new Thread() {
			...
			public void run() {
				...
				try {
					...
					try {
						mo = new MessageObject(Description.NORMAL_RESPONSE, new Long(response.getString(JSON_RESULT)));
					} catch (final NumberFormatException e1) {
						mo = new MessageObject(Description.ERROR, e1);
					} catch (final JSONException e1) {
						mo = new MessageObject(Description.ERROR, e1);
	...
	public void callBoolean(final String method, final OnBooleanResultListener listener, final Object... params) {
		final Handler handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				if (msg == null || msg.obj == null) {
					...
					final MessageObject mo = (MessageObject) msg.obj;
					if (mo.description == Description.ERROR) {
						final Exception e = (Exception) mo.content;
		...
		final Thread thread = new Thread() {
			...
			public void run() {
				...
				try {
					response = doRequest(method, params);
					mo = new MessageObject(Description.NORMAL_RESPONSE, new Boolean(response.getBoolean(JSON_RESULT)));
				} catch (final JSONRPCException e) {
					mo = new MessageObject(Description.ERROR, e);
				} catch (final JSONException e) {
					try {
						mo = new MessageObject(Description.NORMAL_RESPONSE, new Boolean(response.getString(JSON_RESULT)));
					} catch (final JSONException e1) {
						mo = new MessageObject(Description.ERROR, e1);
					}
				}
	...
	public void callDouble(final String method, final OnDoubleResultListener listener, final Object... params) {
		...
		final Thread thread = new Thread() {
			...
			public void run() {
				...
				try {
					response = doRequest(method, params);
					mo = new MessageObject(Description.NORMAL_RESPONSE, new Double(response.getDouble(JSON_RESULT)));
				} catch (final JSONRPCException e) {
					mo = new MessageObject(Description.ERROR, e);
				} catch (final JSONException e) {
					try {
						mo = new MessageObject(Description.NORMAL_RESPONSE, new Double(response.getString(JSON_RESULT)));
					} catch (final NumberFormatException e1) {
						mo = new MessageObject(Description.ERROR, e1);
					} catch (final JSONException e1) {
						mo = new MessageObject(Description.ERROR, e1);
	...
	public void callJSONObject(final String method, final OnJSONObjectResultListener listener, final JSONObject params) {
		...
		final Thread thread = new Thread() {
			...
			public void run() {
				...
				try {
					response = doRequest(method, params);
					mo = new MessageObject(Description.NORMAL_RESPONSE, response.getJSONObject(JSON_RESULT));
				} catch (final JSONRPCException e) {
					mo = new MessageObject(Description.ERROR, e);
				} catch (final JSONException e) {
					mo = new MessageObject(Description.ERROR, e);
	...
	public void callJSONArray(final String method, final OnJSONArrayResultListener listener, final Object... params) {
		...
		final Thread thread = new Thread() {
			...
			public void run() {
				...
				try {
					response = doRequest(method, params);
					mo = new MessageObject(Description.NORMAL_RESPONSE, response.getJSONArray(JSON_RESULT));
				} catch (final JSONRPCException e) {
					mo = new MessageObject(Description.ERROR, e);
				} catch (final JSONException e) {
					mo = new MessageObject(Description.ERROR, e);
