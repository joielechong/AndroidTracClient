...
my $sth1 = $dbh->prepare("SELECT co.id,cn,voornaam,achternaam,tussenvoegsel,geboortedatum,company,webpagina FROM contacts AS co,catcontact AS cc, categories AS ca WHERE ca.categorie='mobielm' and ca.id=cc.cat_id and cc.contact_id=co.id");
