{
    ...
    BEGIN {
        $ETCS::DRU::VERSION = "0.62";
#
#    0.62 Fix van reccount (bleek 3 bytes ipv 2)
#    0.61 Fix van setFilter
#    0.6 Filter op NID_MESSAGE 
#    0.5 Kleine bug fix
...
#        print unpack("H*",$header),"\n";
        bless $self;
        $self->{reccount} = unpack("V*", pack("H*",unpack("H*",substr($header,0x2c))."00"));
        $self->{startrecord} = 0;
        $self->{filterState}=0;
